...
import {
  ...
  ArrowRight, ArrowLeft, Check, Phone, Mail, MapPin, Sparkles,
  Star, Euro,
} from "lucide-react";
  ...
  return [
    {
      ...
      includes: [
        "Hauptgericht nach Wahl (3 Optionen)",
        "Beilagen-Buffet",
    ...
    },
    {
      ...
      includes: [
        "Vorspeisen-Auswahl (2 Optionen)",
        "Hauptgericht (4 Optionen inkl. vegetarisch)",
        "Dessert-Buffet",
        "Getränke-Flatrate (Soft + Bier)",
        "Servicepersonal (1 pro 25 Gäste)",
        "Tisch-Dekoration Standard",
    ...
    },
    {
      ...
      includes: [
        ...
        "Getränke-Flatrate (komplett)",
        "Servicepersonal (1 pro 15 Gäste)",
        "Individuelle Tisch-Dekoration",
        "Premium-Geschirr & Kristallgläser",
        "Auf-/Abbau inklusive",
      ],
    },
  ];
                    ...
                    return (
                      <label
                        ...
                        className={`relative cursor-pointer rounded-xl border-2 p-5 transition-all hover:shadow-md ${
                          active ? tierColors[pkg.tier] : "border-border"
                        } ${!fits ? "opacity-60" : ""}`}
                      >
                        ...
                        {!fits && (
                          <p className="mt-2 text-xs text-accent">Über Ihrem Budget von {budget[0]} €/Person</p>
                        )}
                      </label>
                    );
                  })}
              ...
              <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                ...
                <Card className="mb-6 border-primary/20 bg-primary/5">
                  <CardContent className="flex flex-wrap gap-4 p-4 text-sm">
                    ...
                    {selectedTier && (
                      <span><strong>Paket:</strong> {packages.find((p) => p.tier === selectedTier)?.name}</span>
                    )}
                  </CardContent>
                </Card>
